...
to store-raster
  ...
  gis:store-dataset patches_out (word "C:/Users/DELL/Desktop/Thesis_backup/NetLogo_Noscaled_Results/TestPatches/" "Normal_Fragstat_PatchFile_" iterate_counter ".txt")
...
to setup-output-file
  if (file-exists? "Normal_Test.csv")
  [
    carefully
      [ file-delete "Normal_Test.csv" ]
      [ print error-message ]
  ]

  file-open "Normal_Test.csv"
  file-type "mru,"
  file-type "vru,"
  file-type "dor,"
  file-print "cgini,"
...
to set-alphas
  set alphas (random-normal 0.536 0.244) ;; control_file: alphaA = normal,0.532,0.261

  while [alphas < 0 or alphas > 1] [set alphas (random-normal 0.536 0.244)]

  set alphaq (random-normal 0.532 0.261) ;; control_file: alphaS = normal,0.536,0.244
